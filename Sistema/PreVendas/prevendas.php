        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                function renderCart() {
                    ...
                    // Declaração correta da variável no início da função
                    let total = 0;

                    cart.forEach((item, index) => {
                        const precoBrutoDaLinha = parseFloat(item.valor) * item.qtd;
                        const descontoTotalDaLinha = parseFloat(item.desconto || 0);
                        const subtotalFinalDaLinha = precoBrutoDaLinha - descontoTotalDaLinha;
                        
                        total += subtotalFinalDaLinha;

                        const descontoGerencialTexto = (descontoTotalDaLinha > 0 && !item.desconto_promocao_desc) 
                            ? `<br><small class="text-danger">Desconto Gerencial: - R$ ${descontoTotalDaLinha.toFixed(2).replace('.', ',')}</small>` 
                            : '';

                        let promocaoTexto = '';
                        if (item.desconto_promocao_desc) 
                        ...
                        const itemHtml = `
                            <div class="item-list-row-c border-bottom mb-2">
                                ...
                                <div class="text-center">
                                    <span class="fw-bold">${parseFloat(item.valor).toFixed(2).replace('.', ',')}</span>
                                    <span class="badge bg-secondary ms-1">x ${item.qtd}</span>
                                </div>
                                <div class="text-center">
                                    ${descontoTotalDaLinha > 0 ? `<s class="text-muted">R$ ${precoBrutoDaLinha.toFixed(2).replace('.', ',')}</s><br>` : ''}
                                    <strong>R$ ${subtotalFinalDaLinha.toFixed(2).replace('.', ',')}</strong>
                                </div>
                ...
                window.addToCart = function(searchIndex) {
                    const itemParaAdicionar = searchResults[searchIndex];

                    if (itemParaAdicionar.Controlado === 'Sim') {
                        mostrarToast('Medicamentos controlados só podem ser adicionados via fluxo de Dispensação.', 'danger');
                        return;
                    }

                    const itemExistente = cart.find(item => item.id === itemParaAdicionar.ID_Produto && item.tipo === 'produto');

                    if (itemExistente) 
                        itemExistente.qtd++;
                    else {
                        cart.push({
                            id: itemParaAdicionar.ID_Produto,
                            tipo: 'produto',
                            nome: itemParaAdicionar.Nome,
                            valor: itemParaAdicionar.Preco_Venda,
                            qtd: 1,
                            desconto: 0
                        });
                    }
                    ...
                    mostrarToast(`${itemParaAdicionar.Nome} adicionado à pré-venda!`, 'success');
                    
                    atualizarCarrinhoComPromocoes();
                }
                    ...
                    fetch('processa_prevenda.php', { method: 'POST', body: formData })
                        ...
                        .then(data => {
                            if (data.sucesso) {
                                ...
                                setTimeout(() => window.location.href = 'prevendas.php', 500);
                            }
                            else 
                ...
                async function atualizarCarrinhoComPromocoes() {
                    if (cart.length === 0) {
                        renderCart(); 
                        return;
                    }
                    ...
                    try {
                        const response = await fetch('calcular_promocoes_prevenda.php', {
                            method: 'POST',
                        ...
                        if (!response.ok) 
                            throw new Error('Falha na comunicação com o servidor de promoções.');

                        const carrinhoDoServidor = await response.json();

                        const mapaPromocoes = new Map(carrinhoDoServidor.map(item => [item.id, item]));

                        cart.forEach(itemLocal => {
                            const itemDoServidor = mapaPromocoes.get(itemLocal.id);

                            if (itemDoServidor) {
                                itemLocal.desconto = parseFloat(itemDoServidor.desconto || 0);
                                itemLocal.desconto_promocao_desc = itemDoServidor.desconto_promocao_desc || null;
                            } 
                            else {
                                itemLocal.desconto = 0;
                                itemLocal.desconto_promocao_desc = null;
                            }
                        });
                    } 
                    catch (error) {
                        console.error("Erro ao aplicar promoções:", error);
                        mostrarToast('Não foi possível verificar as promoções.', 'warning');
                    } 
                    finally {
                        renderCart();
                    }
                ...
                function updateCartTotalQuantity() {
                    const totalQty = cart.reduce((sum, item) => sum + item.qtd, 0);
