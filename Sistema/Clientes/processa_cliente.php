...
try {
    if ($id_cliente) {
        if (!empty($senha)) {
            ...
            $stmt_cliente = $conn->prepare("UPDATE CLIENTES SET Nome = ?, Tipo = ?, Tel = ?, Email = ?, Status = ?, OBS = ?, Senha = ? WHERE ID_Cliente = ?");
            $stmt_cliente->bind_param("sssssssi", $nome, $tipo_pessoa, $tel, $email, $status, $obs, $senha_hash, $id_cliente);
        } 
        else {
            $stmt_cliente = $conn->prepare("UPDATE CLIENTES SET Nome = ?, Tipo = ?, Tel = ?, Email = ?, Status = ?, OBS = ? WHERE ID_Cliente = ?");
            $stmt_cliente->bind_param("ssssssi", $nome, $tipo_pessoa, $tel, $email, $status, $obs, $id_cliente);
        }
        ...
        $senha_hash = password_hash($senha, PASSWORD_DEFAULT);
        $stmt_cliente = $conn->prepare("INSERT INTO CLIENTES (Nome, Tipo, Tel, Email, Senha, Status, OBS) VALUES (?, ?, ?, ?, ?, ?, ?)");
        $stmt_cliente->bind_param("sssssss", $nome, $tipo_pessoa, $tel, $email, $senha_hash, $status, $obs);
