<?php
session_start();
error_reporting(E_ALL);
ini_set('display_errors', 1);

include "../../Dev/Exec/config.php";
include DEV_PATH . 'Exec/conexao.php';
include DEV_PATH . 'Exec/logs.php';
include DEV_PATH . "Exec/validar_sessao.php";
include DEV_PATH . "Exec/validar_acesso.php";

$data_inicio = $_GET['data_inicio'] ?? date('Y-m-d', strtotime('-7 days'));
$data_fim = $_GET['data_fim'] ?? date('Y-m-d');
$id_caixa = $_GET['id_caixa'] ?? '';

$sql = "SELECT
            CA.ID_CaixaAberto,
            CA.Data_Abertura,
            CA.Data_Fechamento,
            CA.Saldo_Inicial,
            CA.Saldo_Final,
            CA.Valor_Vendido,
            C.Caixa AS Nome_Caixa,
            F.Nome AS Nome_Funcionario
        FROM CAIXAS_ABERTOS AS CA
        JOIN CAIXAS AS C ON CA.ID_Caixa = C.ID_Caixa
        JOIN FUNCIONARIOS AS F ON CA.ID_Funcionario = F.ID_Funcionario
        WHERE CA.Data_Fechamento IS NOT NULL"; 

$params = [];
$types = '';

$sql .= " AND DATE(CA.Data_Fechamento) BETWEEN ? AND ?";
$types .= 'ss';
$params[] = $data_inicio;
$params[] = $data_fim;

if (!empty($id_caixa)) {
    $sql .= " AND C.ID_Caixa = ?";
    $types .= 'i';
    $params[] = $id_caixa;
}

$sql .= " ORDER BY CA.Data_Fechamento DESC";

$stmt = $conn->prepare($sql);
$stmt->bind_param($types, ...$params);
$stmt->execute();
$result = $stmt->get_result();
$caixas_fechados = $result->fetch_all(MYSQLI_ASSOC);

// ----- CARDS DE RESUMO -----
$total_vendido_periodo = 0;
foreach ($caixas_fechados as $caixa) {
    $total_vendido_periodo += $caixa['Valor_Vendido'];
}
$numero_fechamentos = count($caixas_fechados);
?>

<!DOCTYPE html>
<html lang="pt-br">
    <head>
        <meta charset="UTF-8">
        <title>Relatório de Caixas</title>
        <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.8/dist/css/bootstrap.min.css" rel="stylesheet">
        <link rel="stylesheet" href="<?php echo DEV_URL ?>CSS/global.css">
    </head>
    <body class="bg-light">
        <!-- Navbar -->
        <?php include_once DEV_PATH . 'Views/sidebar.php'?>

        <div class="content d-flex flex-column min-vh-100">
            <div class="content flex-grow-1">
                <!-- Banner -->
                <div class="container-fluid bg-secondary text-white text-center p-4 no-print">
                    <h3>Relatório de Caixas Fechados</h3>
                </div>
                <div class="container p-4">
                    <div class="card card-body mb-4 no-print">
                        <form method="GET" action="relatorio_caixas.php">
                            <div class="row align-items-end">
                                <div class="col-md-3">
                                    <label for="data_inicio">Data Início:</label>
                                    <input type="date" name="data_inicio" id="data_inicio" class="form-control" value="<?= htmlspecialchars($data_inicio) ?>" required>
                                </div>
                                <div class="col-md-3">
                                    <label for="data_fim">Data Fim:</label>
                                    <input type="date" name="data_fim" id="data_fim" class="form-control" value="<?= htmlspecialchars($data_fim) ?>" required>
                                </div>
                                <div class="col-md-3">
                                    <label for="id_caixa">Filtrar por Caixa:</label>
                                    <select name="id_caixa" id="id_caixa" class="form-select">
                                        <option value="">Todos os Caixas</option>
                                        <?php
                                        $caixas_result = $conn->query("SELECT ID_Caixa, Caixa FROM CAIXAS WHERE StatusCadastrado = 'Ativo' ORDER BY Caixa");
                                        while ($caixa_opt = $caixas_result->fetch_assoc()) {
                                            $selected = ($id_caixa == $caixa_opt['ID_Caixa']) ? 'selected' : '';
                                            echo "<option value='{$caixa_opt['ID_Caixa']}' {$selected}>{$caixa_opt['Caixa']}</option>";
                                        }
                                        ?>
                                    </select>
                                </div>
                                <div class="col-md-3">
                                    <button type="submit" class="btn btn-primary w-100">Filtrar</button>
                                </div>
                            </div>
                        </form>
                    </div>

                    <div class="row mb-4">
                        <div class="col-md-6">
                            <div class="card text-center">
                                <div class="card-body">
                                    <h5 class="card-title">Nº de Fechamentos no Período</h5>
                                    <p class="card-text fs-4"><?= $numero_fechamentos ?></p>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="card text-center">
                                <div class="card-body">
                                    <h5 class="card-title">Total Vendido (Soma dos Caixas)</h5>
                                    <p class="card-text fs-4">R$ <?= number_format($total_vendido_periodo, 2, ',', '.') ?></p>
                                </div>
                            </div>
                        </div>
                    </div>

                    <div class="card">
                        <div class="card-header d-flex justify-content-between align-items-center">
                            Histórico de Fechamentos no Período
                            <button onclick="window.print()" class="btn btn-outline-secondary btn-sm no-print">Imprimir Relatório</button>
                        </div>
                        <div class="table-responsive">
                            <table class="table table-striped table-hover mb-0">
                                <thead class="table-dark">
                                    <tr>
                                        <th>Caixa</th>
                                        <th>Operador</th>
                                        <th>Abertura</th>
                                        <th>Fechamento</th>
                                        <th class="text-end">Saldo Inicial</th>
                                        <th class="text-end">Valor Vendido</th>
                                        <th class="text-end">Saldo Final</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <?php if (count($caixas_fechados) > 0): ?>
                                        <?php foreach ($caixas_fechados as $caixa): ?>
                                            <tr>
                                                <td><?= htmlspecialchars($caixa['Nome_Caixa']) ?></td>
                                                <td><?= htmlspecialchars($caixa['Nome_Funcionario']) ?></td>
                                                <td><?= date('d/m/Y H:i', strtotime($caixa['Data_Abertura'])) ?></td>
                                                <td><?= date('d/m/Y H:i', strtotime($caixa['Data_Fechamento'])) ?></td>
                                                <td class="text-end">R$ <?= number_format($caixa['Saldo_Inicial'], 2, ',', '.') ?></td>
                                                <td class="text-end fw-bold text-success">R$ <?= number_format($caixa['Valor_Vendido'], 2, ',', '.') ?></td>
                                                <td class="text-end">R$ <?= number_format($caixa['Saldo_Final'], 2, ',', '.') ?></td>
                                            </tr>
                                        <?php endforeach; ?>
                                    <?php else: ?>
                                        <tr><td colspan="7" class="text-center">Nenhum fechamento de caixa encontrado para o período e filtros selecionados.</td></tr>
                                    <?php endif; ?>
                                </tbody>
                            </table>
                        </div>
                    </div>
                </div>
            </div>

            <!-- Footer -->
            <?php include_once DEV_PATH . 'Views/footer.php'?>
        </div>

        <!-- Toast -->
        <div class="toast-container position-fixed top-0 end-0 p-3">
            <div id="liveToast" class="toast" role="alert" aria-live="assertive" aria-atomic="true">
                <div class="toast-header">
                <strong class="me-auto" id="toastTitulo">Notificação</strong>
                <button type="button" class="btn-close" data-bs-dismiss="toast" aria-label="Close"></button>
                </div>
                <div class="toast-body" id="toastCorpo">
                </div>
            </div>
        </div>
        
        <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js"></script>
        <script src="<?= DEV_URL ?>JS/toast.js"></script>
        <script>
            <?php
            if (isset($_SESSION['msg']) && is_array($_SESSION['msg'])) {
                $texto = addslashes($_SESSION['msg']['texto']);
                $tipo = $_SESSION['msg']['tipo'];
                
                echo "mostrarToast('{$texto}', '{$tipo}');";

                unset($_SESSION['msg']);
            }
            ?>
        </script>
    </body>
</html>